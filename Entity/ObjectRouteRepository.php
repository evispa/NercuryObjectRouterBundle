...
class ObjectRouteRepository extends EntityRepository
{
    ...
    public function getObjectRoutes($objectType, $objectId, $locale)
    {
        $q = $this->_em->createQuery(
            'SELECT r FROM ObjectRouterBundle:ObjectRoute r WHERE r.object_type = ?1 AND r.object_id = ?2 AND r.lng = ?3'
        );
    ...
    {
        $q = $this->_em->createQuery(
            'DELETE from ObjectRouterBundle:ObjectRoute r WHERE r.object_id = ?1 AND r.object_type = ?2 AND r.lng = ?3'
        );
    ...
    public function getRoutingItems($objectType, $objectId)
    {
        return $this->_em
            ->createQuery(
                'SELECT r FROM ObjectRouterBundle:ObjectRoute r WHERE r.object_type = :object_type AND r.object_id = :object_id'
            )
